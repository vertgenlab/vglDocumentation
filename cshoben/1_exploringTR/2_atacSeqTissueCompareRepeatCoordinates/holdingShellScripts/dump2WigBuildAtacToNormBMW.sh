#!/bin/bash
#SBATCH -J dump2Wig
#SBATCH -o %Adump2Wig%A.out
#SBATCH -e %Adump2Wig%A.err
#SBATCH --mem 80G
#SBATCH -p scavenger
...
### Code for running bwa alignment and output to sorted sam file. Use bwa 7.17
module load samtools/1.10
/hpc/group/vertgenlab/crs70/software/bwa/bwa-0.7.17/bwa mem $3 $1_1.fastq.gz $1_2.fastq.gz> $1$2.mem.sam
echo "bwa done"
samtools sort ./$1$2.mem.sam > $1$2.mem.sorted.bam
...
echo "samToWig is done where input was: $1 (SRR#) $2 (shorthand ref) $3 (unused here) $4 (path to chrom.sizes file)"
