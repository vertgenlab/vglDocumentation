...
cat "$p"_master.tab | awk ' BEGIN { FS = "\t"} {if ($26 >= 0.6) print $1, $3, $8, $2}' | tail -n +2 | tr " " "\t" > /work/crs70/geneRegLabelingOfEstrs/tsv"$p"Temp.bed
...
awk '{ if ($5 > 2000) print}' ./compareOnName"$p"Hg19.bed > enhancerCompareOnName0.6"$p"Hg19.bed
awk '{ if ($5 <= 2000) print}' ./compareOnName"$p"Hg19.bed > promoterCompareOnName0.6"$p"Hg19.bed
