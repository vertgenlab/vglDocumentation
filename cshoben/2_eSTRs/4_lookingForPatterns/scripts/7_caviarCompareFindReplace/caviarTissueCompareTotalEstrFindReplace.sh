...
while read p; do
	...
	awk 'BEGIN { FS = "\t"} {if ($26 >= 0.3) print $0}' /hpc/group/vertgenlab/crs70/publicData/eSTRs/caviarTissueCompare/"$p"FindAndReplacedMaster.tsv > "$p"TempCaviar_master.txt
...
cat tempAllTissueFractionTotalEstr.txt | tr " " "\t" | sort -k3 -g > /hpc/group/vertgenlab/crs70/publicData/eSTRs/caviarTissueCompare/allTissueFractionTotalEstrFindReplace.txt 
