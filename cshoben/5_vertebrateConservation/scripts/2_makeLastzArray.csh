...
grep -v $1 /hpc/group/vertgenlab/RefGenomes/vertCons/species.list > speciesNo$1.list
#make a species.list file that doesn't have the reference so you don't end up aligning reference against itself

foreach line (`cat speciesNo"$1".list`)
		...
		foreach t (`ls -1 $targetDir | grep 'chr*'`)
			...
			foreach q (`ls -1 $queryDir`)
				echo $q
				set qNAME = $q:r
				echo "$lastz $targetDir/$t $queryDir/$q --output=$outDir/$tNAME/$qNAME.$tNAME.axt --scores=$matrix --format=axt O=600 E=150 T=2 M=254 K=4500 L=4500 Y=15000" >> lastzJobs.txt
			end
		end
end
echo DONE
