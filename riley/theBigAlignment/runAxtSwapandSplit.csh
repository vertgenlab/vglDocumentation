...
set query = Doris_Troglodytes
set pairwiseDir = /data/lowelab/RefGenomes/Great_Ape_Diversity/$query/Doris_Troglodytes_Chrom/alignments
...
set panTro6Size = /data/lowelab/RefGenomes/Great_Ape_Diversity/$query/Doris_Troglodytes_panTro6Mask_Sub.chrom.sizes
set axtFilter = /data/lowelab/cl454/bin/x86_64/axtFilter
set humanGenome = /data/lowelab/RefGenomes/hg38/hg38.2bit
set chimpGenome = /data/lowelab/RefGenomes/Great_Ape_Diversity/$query/Doris_Troglodytes_panTro6Mask_Sub.2bit
...
foreach hLine(`cat $hg38NoGap | tr '\t' '.' | grep -v 'fix' | grep -v 'chrUn' | grep -v 'alt' | grep -v 'random' | grep -v 'chrM'`)
	...
	mkdir -p chainingOutput/$hChrom/$cChrom

	$axtFilter $pairwiseDir/$hChrom/$cChrom.$hChrom.axt -t=$cChrom -tStartMin=$cStart -tEndMax=$cEnd \
	| axtSwap /dev/stdin $panTro6Size $hg38Size /dev/stdout \
	| $axtFilter /dev/stdin -t=$hChrom -tStartMin=$hStart -tEndMax=$hEnd \
	...
	| chainAntiRepeat  -minScore=$minScore $humanGenome $chimpGenome /dev/stdin chainingOutput/$hChrom/$cChrom/$hName.$cName.chain
end
